...
- **外部ループ**: Claude Code の `/loop` から `/outer-loop` を反復実行
  （1 反復 = 最大 1 feature。`docs/09_external_loop.md`）。
...
- **Codex 用 skills**: `.agents/skills/`（`.claude/skills/` と同等の 13 skill）。
...
- `docs/09_external_loop.md` — /loop による外部ループ設計（Claude Code 固有部分と共通部分）
